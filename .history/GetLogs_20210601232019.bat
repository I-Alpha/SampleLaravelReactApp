...
docker logs samplelaravelreactapp_laravel.test_1 > DockerlogConts/ainer_log.txt 2>&1

FOR /F "tokens=* USEBACKQ" %%F IN (`docker inspect --format='{{.LogPath}}' samplelaravelreactapp_laravel.test_1`) DO (
SET var=%%F  
)
if "%1"=="file" (docker logs samplelaravelreactapp_laravel.test_1 > Dockerlogs/Container_log.txt && %var% > Dockerlogs/log.txt && exit) ^ 
