...
if [ ! "${CID}" ]; then
  ...
  echo "Samplereactapp_laravel.test_1 container not running"  ( docker-compose logs -t > Dockerlogs/Container_log.log & docker stats --no-stream  > Dockerlogs/DockerStats.log ) 
fi
unset CID